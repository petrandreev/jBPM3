...
public class DispatcherAndJobExecutorMethodTest extends TestCase {
  ...
  public void testGetWaitPeriod() {
    ...
    assertTrue("The due date is past the idle interval, we expect a wait period of "
      + currentIdleInterval + " not " + waitPeriod, waitPeriod == currentIdleInterval);
    ...
    waitPeriod = DispatcherThread.getWaitPeriod((int) currentIdleInterval, nextDueDate);
    assertTrue("The due date is past the idle interval, we expect a wait period of approximately "
      + 10 * 1000 + " not " + waitPeriod, 9 * 1000 <= waitPeriod && waitPeriod <= 11 * 1000);
  }
